...
class ScheduleService {
    ...
    static async checkDuplicate({subject, day, section_order}) {
        ...
        let schedules = JSON.parse(await get(SCHEDULES_REDIS_KEY)) ?? await ScheduleService.getSchedules({raw: true});
        let scheduling = JSON.parse(await get(SCHEDULING_REDIS_KEY)) ?? [];
        let constraint = JSON.parse(await get(SCHEDULING_CONSTRAINT_REDIS_KEY)) ?? await SubjectService.getSubjects({raw: true});
        ...
        const foundSubjectSchedulingIndex = scheduling.findIndex(el => el.day === day && el.section_order === section_order);
        ...
        if (foundSubjectSchedulingIndex >= 0) {
            ...
            if (foundOldConstraintIndex === -1) {
                constraint.push({id: scheduling[foundSubjectSchedulingIndex].subject, number_of_period: 1});
            } else {
            ...
            const numberDuplicate = schedules.filter(el => el.subject === subject && el.day === day && el.section_order === section_order);
